...
import jp.techacademy.shunsuke.kino.apiapp.databinding.RecyclerFavoriteBinding
...
class FavoriteItemViewHolder(private val binding: RecyclerFavoriteBinding) :
    ...
    fun bind(favoriteShop: FavoriteShop, position: Int, adapter: FavoriteAdapter) {
        binding.rootView.apply {
            ...
            // クリック時のイベントリスナーを割り当て
            setOnClickListener {
                adapter.onClickItem?.invoke(favoriteShop.id, favoriteShop.imageUrl, favoriteShop.name, favoriteShop.address, favoriteShop.url)
            }
        }
